...
| 包 | 作用 |
| --- | --- |
| `dsh-filetree` | **宿主端插件**(Node):通过 dsh web 服务器提供只读 `/filetree/*` JSON 接口 —— 目录列表、文件读取、递归搜索、git 状态。**零依赖。** |
| `dsh-client-ui-filetree` | **浏览器端插件**(TS/TSX):悬浮 >/< 按钮和右侧文件树抽屉 —— 懒加载虚拟化树、VS Code 风格层级线、搜索、CodeMirror 预览、git 装饰。 |
...
- 启动图:`GET /` → `window.__DSH_BOOT__` 包含 `dsh-client-ui-filetree`。

## 开发

- 源码就在本目录;已安装副本位于 `~/.dsh/profiles/web/node_modules/` —— 浏览器安装是**junction**(指向源码,构建即热更新);宿主是拷贝。
- 浏览器:`cd dsh-client-ui-filetree && npm run dev`(watch + 同步)、`npm run bundle`(压缩一次性构建)、`npm run types`(生成 `lib/types/` 声明)、`npm run typecheck`。
- 宿主:编辑 `dsh-filetree/lib/index.js`,然后拷贝到 `node_modules/dsh-filetree-v<N>/lib/index.js` 并递增名字以免重启部署。
